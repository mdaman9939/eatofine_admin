...
export function AdminExpenseOrdersTable({ rows }: { rows: AdminExpenseOrderRow[] }) {
  ...
  const grandTotal = useMemo(() => filtered.reduce((s, r) => s + (Number(r.total_expense) || 0), 0), [filtered]);
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-slate-900">Expense — Order-wise</h2>
          <p className="text-xs text-slate-500 mt-0.5">What the platform spends per order · total <span className="font-semibold text-rose-600">{inr(grandTotal)}</span></p>
        </div>

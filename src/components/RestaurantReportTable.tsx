...
function OrderChart({ yearly }: { yearly: Array<{ year: number; total: number }> }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-slate-900">📊 Order Statistics</h2>
        <span className="text-xs text-slate-500">Average order value: <span className="font-semibold text-slate-700">{inr(avg)}</span></span>
      </div>
      <div className="flex items-end gap-6 h-48 px-2">
        {yearly.map((y) => (
          <div key={y.year} className="flex-1 flex flex-col items-center justify-end gap-2 group">
            ...
            <div className="w-full max-w-[40px] rounded-t bg-gradient-to-t from-sky-400 to-sky-300 hover:from-sky-500 transition-colors" style={{ height: `${Math.max(4, (y.total / max) * 160)}px` }} />
            <span className="text-xs text-slate-500 font-medium">{y.year}</span>
          </div>
        ))}
      </div>
      <div className="mt-3 flex items-center justify-center gap-2 text-xs text-slate-500">
        <span className="inline-block w-3 h-3 rounded-sm bg-sky-300" /> Total Order Amount
      </div>
    </div>
  );
}
...
export function RestaurantReportTable({ rows, yearly }: { rows: RestaurantReportRow[]; yearly: Array<{ year: number; total: number }> }) {
  ...
  const csv = useMemo(() => {
    ...
    const body = filtered.map((r, i) =>
      [i + 1, r.name ?? "", r.total_food, r.total_order, r.total_order_amount, r.total_discount, r.total_admin_commission, r.total_vat, r.avg_rating, r.rating_count]
        .map((v) => {
          ...
          <table className="w-full text-sm whitespace-nowrap">
            <thead className="bg-slate-50 text-left text-[11px] uppercase tracking-wider text-slate-500 border-b border-slate-200">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-right">Total Food</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.length === 0 ? (
                ...
                <tr key={r.restaurant_id} className="hover:bg-emerald-50/40 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right tabular-nums text-slate-800">{r.total_food}</td>
                  <td className="px-4 py-3 text-right tabular-nums text-slate-800">{r.total_order}</td>
                  <td className="px-4 py-3 text-right tabular-nums font-semibold text-slate-900">{inr(r.total_order_amount)}</td>
                  <td className="px-4 py-3 text-right tabular-nums text-rose-600">{inr(r.total_discount)}</td>
                  <td className="px-4 py-3 text-right tabular-nums text-slate-700">{inr(r.total_admin_commission)}</td>
                  <td className="px-4 py-3 text-right tabular-nums text-slate-700">{inr(r.total_vat)}</td>
                  <td className="px-4 py-3">

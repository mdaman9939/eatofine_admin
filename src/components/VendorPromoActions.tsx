...
type PromoStatus = "pending" | "approved" | "rejected" | "live" | "paused" | "expired" | "draft";
type Mode = "approve" | "reject";

export function VendorPromoActions({ id, status }: { id: number; status: PromoStatus }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = await fetch(`/api/admin/vendor-promotions/${id}/${mode}`, {
        method: "PATCH",
  ...
  function togglePause(paused: boolean) {
    setError(null);
    startTransition(async () => {
      const res = await fetch(`/api/admin/vendor-promotions/${id}/pause`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
  ...
  const isLiveOrApproved = status === "approved" || status === "live";
      ...
      <div className="inline-flex items-center gap-1.5">
        {isPending && (
          <>
            <button
              ...
              disabled={pending}
              className="cursor-pointer rounded-md px-2.5 py-1 text-xs font-semibold bg-gradient-to-b from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white shadow-sm hover:shadow-md hover:shadow-emerald-500/20 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Approve
            </button>
            <button
              ...
              onClick={() => open("reject")}
              disabled={pending}
              className="cursor-pointer rounded-md px-2.5 py-1 text-xs font-semibold bg-gradient-to-b from-rose-600 to-rose-700 hover:from-rose-500 hover:to-rose-600 text-white shadow-sm hover:shadow-md hover:shadow-rose-500/20 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Reject
            </button>
          </>
        ...
        {isLiveOrApproved && (
          <button
            ...
            disabled={pending}
            className="cursor-pointer rounded-md px-2.5 py-1 text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {pending ? "…" : "Pause"}
          </button>
        )}
        ...
        {!isPending && !isLiveOrApproved && !isPaused && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl ring-1 ring-slate-200 w-full max-w-md overflow-hidden">
            <div className={`px-5 py-3 text-white flex items-center justify-between bg-gradient-to-r ${mode === "approve" ? "from-emerald-600 to-emerald-700" : "from-rose-600 to-rose-700"}`}>
              <h3 className="text-sm font-semibold tracking-wide">
                {mode === "approve" ? "Approve promotion" : "Reject promotion"}
              </h3>
            ...
            <div className="px-5 py-4 space-y-3">
              <p className="text-sm text-slate-600">
                {mode === "approve"
                  ? "Approve this promotion so it can go live on the vendor's restaurant. Remarks are optional and shared with them."
                  : "Reject this promotion. Remarks are required and shown to the vendor so they know what to fix."}
              </p>
              <label className="block">
                ...
                <textarea
                  ...
                  placeholder={
                    mode === "approve"
                      ? "Optional — e.g. 'Reviewed against marketing policy, looks good.'"
                      : "Required — e.g. 'Banner imagery violates branding guidelines, please re-upload.'"
                  }
